#!/usr/bin/env python3
"""
Invoice Generator for Muthu Cleaning and Maintenance Services Ltd
...
# Configuration
COMPANY_NAME = "MUTHU CLEANING AND MAINTENANCE"
COMPANY_NAME_LINE2 = "SERVICES LTD"
COMPANY_ADDRESS_LINE1 = "Lefkonos, 7A"
COMPANY_ADDRESS_LINE2 = "6013, Larnaca"
COMPANY_ADDRESS_LINE3 = "Cyprus"
COMPANY_REG = "HE410185"  # Update if different
TAX_RATE = 0.09
CURRENT_DATE = "31/01/26"  # Last day of January 2026
...
def generate_invoice_pdf(invoice_num, guest_name, property_name, nights, price, output_path):
    ...
    # Row 1: Company Name on left (two lines)
    c.setFont("Helvetica-Bold", 16)
    c.setFillColor(PRIMARY_COLOR)
    c.drawString(left_margin, header_y, COMPANY_NAME)
    c.drawString(left_margin, header_y - 18, COMPANY_NAME_LINE2)
    ...
    # Highlighted thank you banner
    banner_width = 130 * mm
    banner_height = 10 * mm
    banner_x = (width - banner_width) / 2
    banner_y = footer_y - 2 * mm

    # Draw highlight background (light blue)
    c.setFillColor(LIGHT_BLUE)
    c.roundRect(banner_x, banner_y, banner_width, banner_height, 2*mm, fill=1, stroke=0)

    # Thank you message (colored text)
    c.setFont("Helvetica-Bold", 12)
    c.setFillColor(PRIMARY_COLOR)
    c.drawCentredString(width / 2, footer_y + 1*mm, "We Appreciate Your Business!")
